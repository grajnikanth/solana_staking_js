...
const main = async () => {
    ...
    // Now deactivate the staking account staked above and check the state of 
    // staking for this account

    const detactivateTx = StakeProgram.deactivate({
        stakePubkey: stakeAccount.publicKey,
        authorizedPubkey: wallet.publicKey
    });
    ...
    console.log("Stake account deactivation transaction sent to Blockchain");
    console.log("Stake account status is ", stakeStatus.state);

    // send transaction to withdraw the SOL which is with the stake account
    // Get the balance out of the staking account
    ...
    console.log("stake account balance withdraw information");
    stakeBalance = await connection.getBalance(stakeAccount.publicKey);
    console.log(`stakeAccount ${stakeAccount.publicKey.toString()} balance is ${stakeBalance/LAMPORTS_PER_SOL}`);
